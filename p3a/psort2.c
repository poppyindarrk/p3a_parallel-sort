...
int main(int argc, char** argv) {
    ...
    for(long i = 0; i<maxthread; i++) {
        pthread_create(&threads[i], NULL, parallel_sort, (void*)i);
    }
    for(int i = 0; i<maxthread; i++) {
        pthread_join(threads[i], NULL);
    }
    // int num_merges = maxthread/2;
    // int size = arrlen/maxthread*2;
    // while(num_merges > 0) {
    //     int sections = maxthread;
    //     for(int i = 0; i<num_merges; i++) {
    //         int low = i * size;
    //         int high;
    //         // if(maxthread %2 == 0 && i==num_merges-1) {  
    //         //     high = arrlen - 1;
    //         // } else {
    //         high = low + size - 1;
    //         // }
